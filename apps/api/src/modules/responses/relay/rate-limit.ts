...
export class ResponsesRateLimitExceededError extends Error {
  constructor(readonly retryAfterSeconds: number) {
    super("Rate limit exceeded");
    this.name = "ResponsesRateLimitExceededError";
  }
}

export class ResponsesRateLimitUnavailableError extends Error {
  constructor(cause: unknown) {
    super("Rate limit service unavailable");
    this.name = "ResponsesRateLimitUnavailableError";
    this.cause = cause;
  }
}
...
function windowInfo(config: ResponsesRelayConfig, nowMs: number) {
...
export async function checkResponsesRateLimit(
  apiKeyId: string,
  ...
  try {
    const result = await redis.eval(
      CHECK_SCRIPT,
      2,
      `responses_rate:total:${apiKeyId}:${bucket}`,
      `responses_rate:success:${apiKeyId}:${bucket}`,
    ...
    if (!Array.isArray(result) || (result[0] !== 1 && result[0] !== "1")) {
      throw new ResponsesRateLimitExceededError(retryAfterSeconds);
    }
  } catch (error) {
    if (error instanceof ResponsesRateLimitExceededError) throw error;
    throw new ResponsesRateLimitUnavailableError(error);
  }
}

export async function recordResponsesRateLimitSuccess(
  apiKeyId: string,
  config: ResponsesRelayConfig,
  nowMs = Date.now(),
): Promise<void> {
  ...
  try {
    const key = `responses_rate:success:${apiKeyId}:${bucket}`;
