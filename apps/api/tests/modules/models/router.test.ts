...
const {
  ...
  mockGetActiveUserModelAccess: vi.fn().mockResolvedValue(null),
...
vi.mock("../../../src/modules/keys/services", () => ({
  getActiveUserModelAccess: mockGetActiveUserModelAccess,
...
describe("models router", () => {
  afterEach(() => {
    ...
    mockGetActiveUserModelAccess.mockResolvedValue(null);
  ...
  it("GET /dashboard/models filters the catalog for regular users", async () => {
    ...
    mockGetActiveUserModelAccess.mockResolvedValueOnce({
      allowAllModels: false,
      includeFutureModels: false,
      allowedModelIds: ["openai:gpt-4o", "removed:stale-model"],
    });
    mockListPublicModels.mockResolvedValueOnce([
      {
        id: "gpt-4o",
    ...
    expect(res.status).toBe(200);
    await expect(res.json()).resolves.toMatchObject({
      data: [{ id: "openai:gpt-4o", provider: "openai" }],
    });
    expect(mockGetActiveUserModelAccess).toHaveBeenCalledWith("user-1");
  });

  it("GET /dashboard/models returns the global catalog for users without active keys", async () => {
    mockGetCurrentUser.mockResolvedValueOnce({
      id: "user-1",
      email: "user@test.com",
      name: "User",
    ...
    mockGetActiveUserModelAccess.mockResolvedValueOnce(null);
    mockListPublicModels.mockResolvedValueOnce([
      {
        id: "gpt-4",
        name: "GPT-4",
